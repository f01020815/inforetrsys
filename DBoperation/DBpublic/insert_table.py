...
def insert_table(class_name, words, values):
    ...
    :param class_name:在DBoperation中定义的class，
                      在使用该函数前必须import对应的class！from DBoperation.models import class_name
    :param words:该值为list[]格式，class中的字段，表中的域，要插入的域；实现方法是将数据插入数据库表的list[n]域中
    :param values:该值为list[]格式，从execl获取或者前段页面获取的值；实现方法是将list[n]数据插入数据库的表中
