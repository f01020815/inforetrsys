...
def DBapproval_agree_delfile(approval_o_path_id, approval_id):
    # 审批：删除同意
    # 通过审批ID获取文档ID
    ...
    for file_list in file_lists:
        file_id = file_list
        # 文档不显示
        UploadFile.objects.filter(file_id=file_id).update(file_show_state=0, file_approval_state=1, file_path_id="")
