...
def DBapproval_find_update(user):
    # 查询所有更新的待审批的条目
    approval_list_update = []
    if user == "admin":
        for res in PathApproval.objects.filter(Q(approval_opt="修改") | Q(approval_opt="原数据")):
            approval_list= []
            ...
            if res.approval_n_file_info:
                for key, value in eval(res.approval_n_file_info).items():
                    res_file_info += " <a href=""/document_show/?file_id=" + str(
                        key) + " target=""_blank""  style=color:red; rel=""nofollow noopener noreferrer"" title=" + value + "><b>报告"+str(i)+"</b></a>"
                    i = i + 1
            if res.approval_o_file_info:
            ...
            approval_list_update.append(approval_list)
    else:
        for res in PathApproval.objects.filter(Q(approval_hostname=user[0]) & Q(approval_ip=user[1]) & Q(approval_mac=user[2])).filter(Q(approval_opt="修改") | Q(approval_opt="原数据")):
            approval_list= []
            approval_list.append(res.approval_opt)
            approval_list.append(res.approval_linetype)
            approval_list.append(res.approval_content)
