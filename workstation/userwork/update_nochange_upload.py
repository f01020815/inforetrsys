...
def update_nochange_upload(path_id, approval_n_list, approval_o_list, request, upload_files):
    # 插入审批表 作为修改数据 new_approval_id_n是修改的审批ID
    # 获取  连接类型 / 交易描述 / 交易类型 / 交易路径 / 备注说明 /hash / 数据库fileid
    data = []
    ...
    # 将 path_id信息 插入到审批表作为新增(修改)
    new_approval_id_n = DBinsert_pathid(data, path_id)
    # 将approval_o_file_info放数据库搜索的file_id
    # 将opt'增加'改为'修改'
    ...
    if upload_files:
        approval_file_info = upload_high_file(path_id, upload_files, request)
    ...
    # 将条目设置为审批中，无法更新
    Path.objects.filter(path_id=str(path_id)).update(path_show_state=1, path_approval_state=0)
